...
const AIConsultant: React.FC = () => {
  ...
  const speakText = async (text: string) => {
    ...
    const cleanText = text
      .replace(/\[TTS \| Voice: .*?\]/g, '') // Remove TTS tags
      .replace(/\[PRE-RECORDED AUDIO\]/g, '') // Remove Pre-recorded tags
      .replace(/[*#_`~\[\]()]/g, '')
      .replace(/<[^>]*>/g, '')
    ...
    if (usePremiumVoice) {
      if (cleanText.length > 500 && !window.confirm("El texto es largo. ¿Usar créditos de voz Premium?")) {
        ...
        try {
          setIsSpeaking(true);
          const { generateSpeech } = await import('../services/edgeTTS');
          // EdgeTTS handles playback directly, no buffer returned
          await generateSpeech(cleanText, 'lina');
          setIsSpeaking(false);
  ...
  const suggestedPrompts = [
    ...
    "Crea una evaluación rápida de Trigonometría"
  ];

...
const StudentManagement: React.FC = () => {
    ...
    const handleDelete = async (uid: string, name: string) => {
        console.log("handleDelete called with:", JSON.stringify({ uid, name })); // DEBUG: Check if uid is valid
        if (!uid) {
            toast.error("Error: ID de estudiante no válido (undefined)");
            return;
        }

        if (confirm(`¿Estás seguro de que deseas eliminar permanentemente a ${name}? Esta acción no se puede deshacer.`)) {
            const result = await deleteStudentProfile(uid);
            console.log("deleteStudentProfile result:", JSON.stringify(result));
            if (result.success) {
    ...
    const handleToggleStatus = async (uid: string, currentStatus: string) => {
        const newStatus = currentStatus === 'pending' || currentStatus === 'rejected' ? 'active' : 'pending';
        const result = await updateUserStatus(uid, newStatus);
        if (result.success) {
            toast.success(`Cuenta ${newStatus === 'active' ? 'aprobada' : 'desactivada'} correctamente`);
            loadStudents();
        } else {
            ...
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <form onSubmit={handleRegister} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-stone-500 mb-1">Grado</label>
                            <select
                        ...
                        <div className="flex items-center gap-2">
                            <input
                                ...
                                onChange={e => setFormData({ ...formData, isBilingual: e.target.checked })}
                                className="rounded"
                            />
                            <label htmlFor="is_bilingual" className="text-xs font-bold text-stone-500">¿Estudiante de Colegio Bilingüe?</label>
                        </div>
                        <div>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-stone-100">
                                {filteredStudents.map((student) => (
                                    <tr key={student.uid || student.id} className="hover:bg-indigo-50/30 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <button
                                                onClick={() => handleToggleStatus(student.uid || student.id, student.account_status)}
                                                className={`px-3 py-1 rounded-full text-xs font-bold transition-all border ${student.account_status === 'pending'
                                            ...
                                            >
                                                {student.account_status === 'pending' ? '⏳ Pendiente' : student.account_status === 'active' || !student.account_status ? '✅ Activo' : '❌ Inactivo'}
                                            </button>
                                        </td>
                                        <td className="px-6 py-4">
                                            <button
                                                onClick={() => handleUpdateLevel(student.uid || student.id, student.level)}
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <button
                                                    onClick={() => handleDelete(student.uid || student.id, student.name)}

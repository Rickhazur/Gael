...
import { CleanWhiteboard, WhiteboardRef } from '../MathMaestro/tutor/CleanWhiteboard';
...
const ArtsTutor: React.FC<ArtsTutorProps> = ({ onNavigate, gradeLevel }) => {
    ...
    // Using CleanWhiteboard for better performance and magic tools
    ...
    React.useEffect(() => {
        ...
        if (missionJson) {
            try {
                ...
                toast.info(isEn ? `🎨 Mission Active: ${mission.title}` : `🎨 Misión Activa: ${mission.title}`);
            } catch (err) {
    ...
    const handleCompleteMission = () => {
        if (activeMission && activeMission.id) {
            try {
                ...
                addCoins(coins, isEn ? `Art Mission: ${activeMission.title}` : `Misión de Arte: ${activeMission.title}`);
                addXP(xp);

                localStorage.removeItem('nova_mission_params');
                setActiveMission(null);
                toast.success(isEn ? "Art Mission accomplished and reward granted!" : "¡Misión de Arte cumplida y recompensa otorgada!");
            } catch (e) {
                console.error("Error completing art mission:", e);
            }
        }
    };
                    ...
                    <div className="w-64 flex flex-col gap-4">
                        ...
                        {activeMission && (
                            <Button
                                ...
                                className="mt-4 bg-kid-yellow hover:bg-kid-yellow/90 text-black font-black border-2 border-black shadow-comic animate-bounce"
                            >
                                <Sparkles className="w-4 h-4 mr-2" />
                                {isEn ? 'Finish Work' : 'Terminar Obra'}
                            </Button>
                        )}

...
import { OpenAITutorSession as GoogleTutorSession } from '../services/ai_service';
...
const LiveClassroom: React.FC<LiveClassroomProps & { subjectId?: string }> = ({ session, studentName, onExit, language, subjectId }) => {
  ...
  const sessionRef = useRef<GoogleTutorSession | null>(null);
  ...
  const playAudio = async (text: string) => {
    try {
      setIsSpeaking(true);
      // EdgeTTS handles playback directly
  ...
  const initSession = async () => {
    ...
    const aiSession = new GoogleTutorSession(apiKey, sysPrompt);
    ...
    aiSession.onMessage = (event) => {
      if (event.type === 'text' && event.text) {
        ...
        if (rawText.includes('[PRE-RECORDED AUDIO]')) {
          cleanText = rawText.replace('[PRE-RECORDED AUDIO]', '').trim();
          shouldSpeak = false;
        } else if (rawText.match(/\[TTS \| Voice: .*?\]/)) {
          cleanText = rawText.replace(/\[TTS \| Voice: .*?\]/, '').trim();
          shouldSpeak = true;
        }

        // Logic split: We likely want to display the text cleanly 
        // But here we are just "playing" it as it streams? 
        // Actually onMessage comes in chunks or full? 
        // OpenAITutorSession sends chunks?
        // Let's check OpenAITutorSession. It sends choice.message.content (full).
        ...
        {status !== 'connected' ? (
          <div className="text-center space-y-12 animate-fade-in z-20 p-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-stone-400 text-sm font-bold uppercase tracking-widest">Powered by ElevenLabs & Google Gemini</p>
            </div>
            <button
              onClick={initSession}

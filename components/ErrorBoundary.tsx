...
const isNetworkOrChunkError = (message: string) => {
    const m = (message || '').toLowerCase();
    return m.includes('chunk') || m.includes('fetch') || m.includes('loading') || m.includes('network') || m.includes('imported module');
};

export const ErrorFallback: React.FC<FallbackProps> = ({ error, resetErrorBoundary }) => {
    const isConnectionError = isNetworkOrChunkError(error.message);
    return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 text-white font-sans">
            <div className="max-w-md w-full bg-slate-800 border border-slate-700 rounded-2xl p-8 shadow-2xl text-center">
                ...
                <h2 className="text-2xl font-black mb-2 text-white">
                    {isConnectionError ? 'Problema de conexión' : 'Algo salió mal'}
                </h2>
                <p className="text-slate-400 mb-6 text-sm">
                    {isConnectionError
                        ? 'Comprueba tu internet y toca Reintentar. Si usas datos móviles, prueba con Wi‑Fi.'
                ...
                {isConnectionError && <div className="mb-8" />}

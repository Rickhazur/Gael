...
import { ShoppingBag, UserCircle2, Sparkles, Check, Coins, Settings2, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Plus, Minus, X, Store, Lock, Key, Star, Trash2, RotateCcw, RotateCw } from 'lucide-react';
...
export const AvatarShop = ({ demoData = null, showAdoptButton = false }: AvatarShopProps) => {
    ...
    const handleBuy = (item: any) => {
        if (coins >= item.cost) {
            ...
            buyAccessory(item);
            equipAccessory(item);
            // Show fitting room after purchase
    ...
    return (
        <>
            ...
            >
                {/* 🗂️ CATEGORY SELECTOR (Sleek Horizontal) */}
                <div className="flex justify-center p-6 shrink-0 z-50">
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-32">
                                {ACCESSORIES
                                    .filter(i => {
                                        ...
                                        if (activeCategory === 'hats') return i.type === 'head';
                                                                ...
                                                                return (
                                                                    <div className="relative group/img">
                                                                        {item.rarity === 'legendary' && (
                                                                            <div className="absolute inset-0 bg-cyan-400 blur-2xl opacity-40 group-hover/img:opacity-70 animate-pulse transition-opacity" />
                                                                        )}
                                                                        <img
                                                                            src={displayIcon}
                                                                            alt={item.name}

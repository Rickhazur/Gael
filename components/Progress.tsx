...
import {
    ...
    generateTeacherEmailLink, getDBAPerformance
} from '../services/supabase';
import { Infraction, ViewState, StoreItem, EducationalPlan, AppMessage } from '../types';
import { generateParentEmailReport } from '../services/ai_service';
import { HeatmapDBA } from './Admin/HeatmapDBA';
...
const Progress: React.FC<ProgressProps> = ({
    ...
    useEffect(() => {
        ...
        const initConfig = async () => {
            try {
                ...
                if (isMounted) setRoomCheckEnabled(enabled);

                // FORCE DEMO DATA FOR SOFIA
                const isSofia = userName === 'luna' || localStorage.getItem('nova_demo_mode') === 'true';
                ...
                if (isSofia) {
                    if (isMounted) {
                        setStudentProgressData({
                            ...
                            sessions: [
                                { lesson_title: 'Misión Espacial: Geometría', started_at: new Date().toISOString(), homework_score: 100, feedback: '¡Excelente trabajo, luna!' },
                                { lesson_title: 'Aventura Histórica', started_at: new Date(Date.now() - 86400000).toISOString(), homework_score: 95, feedback: 'Muy buena investigación.' }
                            ]
                        });
                ...
                if (userRole === 'ADMIN') {
                    ...
                    if (isMounted) {
                        ...
                        setIsLoadingProgress(false); // Success
                    }
                } else if (userRole === 'STUDENT' && !userId) {
                    // STOP LOADING IF NO USER ID
        ...
        // SAFETY TIMEOUT - Force stop loading after 1.5s
        const safetyTimeout = setTimeout(() => {
            if (isMounted) {
                setIsLoadingProgress((current) => {
                    if (current) {
                        // Check if we need to inject demo data because real fetch stalled
                        const isSofia = userName === 'luna';
                        if (isSofia || !studentProgressData) {
                            setStudentProgressData({
                                summary: { completedSessions: 12, averageScore: 98, canContinue: true },
                                sessions: [{ lesson_title: 'Misión Espacial', started_at: new Date().toISOString(), homework_score: 100, feedback: 'Excelente (Backup Data)' }]
                            });
                        }
                        return false;
                    }
    ...
    }, [userRole, studentsList, userId, userName]);
            ...
            <div className="max-w-7xl mx-auto space-y-8 pb-20 font-sans">
                ...
                {adminTab === 'REPORTS' && (
                    <div className="space-y-8">
                        ...
                        {/* DBA HEATMAP SECTION */}
                        <div className="animate-fade-in bg-white rounded-3xl border border-stone-200 shadow-sm p-8">
                            <HeatmapDBA />
                        </div>
                    </div>
                )}
                        ...
                        <div className="md:col-span-8">
                            {plans.find(p => p.id === activePlanId) && (
                                <div className="bg-white rounded-3xl border border-stone-200 p-8">
                                    <input value={plans.find(p => p.id === activePlanId)?.name} onChange={e => updatePlanDetails('name', e.target.value)} className="text-2xl font-bold w-full mb-2" />
                                    <input value={plans.find(p => p.id === activePlanId)?.description} onChange={e => updatePlanDetails('description', e.target.value)} className="text-sm w-full mb-4" />
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mt-6">
                                        {[
                                            ...
                                            <label key={item.id} className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer ${plans.find(p => p.id === activePlanId)?.allowedViews.includes(item.id) ? 'bg-emerald-50 border-emerald-500' : 'bg-white'}`}>
                                                <input type="checkbox" className="hidden" checked={plans.find(p => p.id === activePlanId)?.allowedViews.includes(item.id)} onChange={() => toggleViewInPlan(item.id)} />
    ...
    if (userRole === 'STUDENT') {
        // PERMANENT DEMO DATA
        const demoSummary = { completedSessions: 12, averageScore: 98, canContinue: true };
        const demoSessions = [
            { lesson_title: 'Misión Espacial: Geometría', started_at: new Date().toISOString(), homework_score: 100, feedback: '¡Excelente trabajo, luna!' },
            { lesson_title: 'Aventura Histórica', started_at: new Date(Date.now() - 86400000).toISOString(), homework_score: 95, feedback: 'Muy buena investigación.' }
        ];
        ...
        // Demo Voice Listener
        useEffect(() => {
            const startSpeaking = () => setIsDemoSpeaking(true);
        ...
        // Bypass loading for Demo User
        if (isLoadingProgress && !isDemoUser) return <div className="text-center p-20"><RefreshCw className="animate-spin" /></div>;

        const summary = studentProgressData?.summary || (isDemoUser ? demoSummary : null);
        const sessions = studentProgressData?.sessions || (isDemoUser ? demoSessions : []);

        return (
            <div className="max-w-4xl mx-auto space-y-8 pb-20 font-sans animate-fade-in relative">

                {/* HEADER */}
                <div className="bg-indigo-600 rounded-3xl p-8 text-white relative min-h-[160px]">
                    {/* DIRECT CHECK: isDemoUser guarantees render for Sofia */}
                    {isDemoUser && (
                        <div className="absolute right-4 top-4 z-[100] border-2 border-transparent">
                            <LinaAvatar state={isDemoSpeaking ? 'speaking' : 'idle'} size={140} />
                            {/* DEBUG VISIBLE ON SCREEN - REMOVE LATER */}
                            <div className="text-[10px] text-white opacity-50 text-center mt-1">Lina Active</div>
                        </div>
                    )}

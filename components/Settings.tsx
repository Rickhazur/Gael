...
const Settings: React.FC<SettingsProps> = ({
    ...
    // Fetch initial preferences from Supabase
    React.useEffect(() => {
        const fetchPrefs = async () => {
            const { supabase } = await import('../services/supabase');
            if (!supabase) return;
            const { data, error } = await supabase.from('profiles').select('whatsapp_notifications_enabled, avatar').eq('id', userId).single();
            if (data && !error) {
                setNotifications(data.whatsapp_notifications_enabled ?? true);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await updateUserProfile(userId, {
                ...
                whatsapp_notifications_enabled: notifications
            });
            onUpdateUser(name);
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="md:col-span-2 space-y-8">
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {AVATARS.map(emoji => (
                                        <button
                                            ...
                                            className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl transition-all border-2 ${selectedAvatar === emoji
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {[6, 7, 8, 9, 10, 11].map((age) => (
                                        <button
                                            ...
                                            className={`w-16 h-16 rounded-xl font-black text-xl transition-all ${studentAge === age
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between p-3 hover:bg-stone-50 rounded-xl transition-colors">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-bold text-stone-700 text-sm">Notificaciones Email</p>
                    ...
                    {message && (
                        <div className={`p-4 rounded-xl border flex items-center gap-3 animate-fade-in ${message.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-rose-50 border-rose-200 text-rose-700'
                            }`}>

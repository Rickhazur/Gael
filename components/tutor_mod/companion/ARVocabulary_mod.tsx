...
const ARVocabulary_mod = ({ onComplete, onBack, personalizedContent, immersionMode = 'bilingual' }: ARVocabularyProps) => {
  ...
  const vocabularyObjects = useMemo<VocabObject[]>(() => {
    if (!personalizedContent || personalizedContent.vocabulary.length === 0) {
      ...
      return [
        { id: 'f1', word: 'Learn', translation: 'Aprender', definition: 'To gain knowledge', category: 'general', emoji: '📖', color: 'from-blue-400 to-indigo-500', difficulty: 'beginner', tip: 'We learn new things every day!' },
        { id: 'f2', word: 'Practice', translation: 'Practicar', definition: 'To repeat to improve', category: 'general', emoji: '🎯', color: 'from-green-400 to-emerald-500', difficulty: 'beginner', tip: 'Practice makes perfect!' },
      ];
    }
      ...
      <motion.div className="flex-1 flex flex-col" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground flex items-center gap-2">
              ...
              {immersionMode === 'standard' ? 'Explorador AR de Vocabulario' : 'AR Vocabulary Explorer'}
            </h2>
            <p className="text-muted-foreground">
              {immersionMode === 'standard'
                ? '¡Toca objetos virtuales para aprender palabras!'
                : 'Touch virtual objects to learn words!'}
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-6">
          ...
          <h3 className="text-xl font-bold text-foreground">
            {immersionMode === 'standard' ? 'Elige tu nivel' : 'Choose Your Level'}
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-lg">
            {(['beginner', 'intermediate', 'advanced'] as const).map((level) => (
              <motion.button
                ...
                className={`p-6 rounded-2xl border-2 transition-all ${level === 'beginner' ? 'bg-green-100 border-green-400 hover:bg-green-200' :
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold text-foreground">
              {immersionMode === 'standard' ? 'Elige una categoría' : 'Choose a Category'}
            </h2>
            <p className="text-muted-foreground capitalize">
              {immersionMode === 'standard'
                ? `Nivel ${difficulty} seleccionado`
    ...
    <motion.div className="flex-1 flex flex-col" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="flex-1 relative bg-gradient-to-br from-indigo-900/20 via-purple-900/20 to-pink-900/20 rounded-3xl overflow-hidden border border-white/10 min-h-[300px]">
            ...
            {currentObjects.map((obj, index) => {
              ...
              return (
                <motion.div
                  ...
                  whileTap={{ scale: 0.9 }}
                >
                  <div className={`relative p-4 rounded-2xl bg-gradient-to-br ${obj.color} shadow-lg ${isSelected ? 'ring-4 ring-white shadow-2xl' : ''
        ...
        >
          ...
          {immersionMode === 'standard' ? '¡Toca los objetos para explorar!' : 'Tap objects to explore!'}
        </motion.div>
      </div>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-lg text-primary font-bold">{selectedObject.translation}</p>
                <p className="text-sm text-muted-foreground">{selectedObject.definition}</p>

                <div className="flex items-center gap-2 mt-2">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${selectedObject.difficulty === 'beginner' ? 'bg-green-100 text-green-700' :

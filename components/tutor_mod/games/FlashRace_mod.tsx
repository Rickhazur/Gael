  ...
  const words = useMemo<FlashWord[]>(() => {
    if (personalizedContent && personalizedContent.vocabulary.length > 0) {
      return personalizedContent.vocabulary.slice(0, 15).map((v, idx) => ({
        id: `pv-${idx}`,
        word: v.word,
        translation: immersionMode === 'standard' ? v.translation : v.definition,
        tags: [v.category],
      ...
      >
        ...
        <div className="flex items-center justify-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-muted rounded-xl">
            ...
            <span className="font-bold">{words.length} {immersionMode === 'standard' ? 'palabras' : 'words'}</span>
          </div>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="p-4 bg-muted rounded-xl">
            <p className="text-sm text-muted-foreground">{immersionMode === 'standard' ? 'Correctas' : 'Correct'}</p>
            <p className="text-2xl font-bold text-success">{correctCount}/{words.length}</p>
          </div>
          <div className="p-4 gradient-secondary rounded-xl">
            <p className="text-sm text-secondary-foreground/80">{immersionMode === 'standard' ? 'Monedas' : 'Coins'}</p>
        ...
        <Button variant="default" onClick={onClose} size="lg">
          ...
          {immersionMode === 'standard' ? 'Continuar' : 'Continue'}
        </Button>
      </motion.div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${timeLeft <= 10 ? 'bg-destructive text-destructive-foreground animate-pulse' : 'bg-muted'
            }`}>
      ...
      <AnimatePresence>
        {showFeedback && (
          <motion.div
            className={`mt-4 p-4 rounded-xl text-center font-semibold ${isCorrect

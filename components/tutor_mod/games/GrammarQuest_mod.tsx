...
const levelNamesEs = ['Isla de Inicio', 'Bosque Mágico', 'Montaña Sabia', 'Volcán de Velocidad', 'Castillo Final'];
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2 px-4 py-2 gradient-secondary rounded-xl text-secondary-foreground">
            ...
            <span className="font-bold">{coins} {immersionMode === 'standard' ? 'monedas' : 'coins'}</span>
          </div>
        </div>

        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-foreground mb-2">
            {immersionMode === 'standard' ? '🗺️ Misión Gramatical' : '🗺️ Grammar Quest'}
          </h2>
          <p className="text-muted-foreground">
            {immersionMode === 'standard' ? '¡Completa niveles para ganar estrellas y monedas!' : 'Complete levels to earn stars and coins!'}
          </p>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <motion.button
                    ...
                    className={`relative z-10 w-20 h-20 rounded-2xl flex flex-col items-center justify-center shadow-medium transition-all ${isUnlocked
                        ? isCompleted
                  ...
                  <div className="flex-1 text-center">
                    <p className={`font-semibold ${isUnlocked ? 'text-foreground' : 'text-muted-foreground'}`}>
                      {immersionMode === 'standard' ? levelNamesEs[index] : name}
                    </p>
                  </div>
                </motion.div>
              );
          ...
          >
            <div className="p-4 bg-destructive/10 rounded-xl mb-4">
              <p className="text-sm text-muted-foreground mb-1">
                {immersionMode === 'standard' ? (
                  challenge.type === 'correct' ? 'Corrige esta oración:' :
                    challenge.type === 'complete' ? 'Completa la oración:' :
                      'Ordena las palabras:'
                ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {immersionMode === 'standard' ? 'Pista' : 'Hint'}
              </Button>
              {!challenge.options && (
                <Button
                  variant="default"
                ...
                >
                  {isCorrect ? (
                    <p className="text-success font-medium">
                      {immersionMode === 'standard' ? `¡Perfecto! +${showHint ? 5 : 10} monedas` : `🎉 Perfect! +${showHint ? 5 : 10} coins`}
                    </p>
                  ) : (
                    <div>
                      <p className="text-foreground font-medium mb-1">
                        {immersionMode === 'standard' ? 'Respuesta correcta:' : 'Correct answer:'}
                      </p>

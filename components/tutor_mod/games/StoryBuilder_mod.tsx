...
const StoryBuilder_mod = ({
  ...
  const t: Record<string, any> = {
    es: {
      ...
      theme: "Tema",
      drag: "👇 Arrastra piezas aquí para crear tu historia",
      available: "Piezas disponibles:",
      review: "Revisar Historia",
      excellent: "¡Excelente historia!",
      good: "¡Buena historia!",
      ...
      edit: "Editar",
      continue: "Continuar",
      completed: "¡Historia Completa!",
      earned: "Has ganado",
      forCreativity: "monedas por tu creatividad",
      tipStarter: "💡 Toda historia necesita un buen comienzo (starter)",
      tipEnding: "💡 No olvides cerrar tu historia con un final (ending)",
      tipConnector: "💡 Usa conectores (sin embargo, entonces, porque) para unir ideas"
    },
    en: {
      back: "Back",
      reset: "Reset",
      title: "Story Builder",
      theme: "Theme",
      drag: "👇 Drag pieces here to create your story",
      ...
      earned: "You earned",
      forCreativity: "coins for your creativity",
      tipStarter: "💡 Every story needs a good beginning (starter)",
      tipEnding: "💡 Don't forget to close your story with an ending",
      tipConnector: "💡 Use connectors (however, then, because) to join ideas"
    }
  };
  ...
  const checkStory = useCallback(async () => {
    ...
    setGameState('review');
  }, [selectedPieces, targetStructure, text]);
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-muted-foreground">
            {text.theme}: <span className="font-semibold text-primary">{theme}</span>
          </p>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="p-3 bg-success/20 rounded-xl text-center">
            <p className="text-sm text-muted-foreground">{text.structure}</p>
            <p className="text-xl font-bold text-success">
              {feedback?.correct}/{feedback?.total}
            </p>
          </div>
          <div className="p-3 gradient-secondary rounded-xl text-center">
            <p className="text-sm text-secondary-foreground/80">{text.coins}</p>
        ...
        {feedback && feedback.tips.length > 0 && (
          <div className="p-3 bg-warning/20 rounded-xl mb-4">
            <p className="font-medium text-foreground mb-2">{text.tips}</p>
  ...
  return (
    ...
    >
      ...
      <p className="text-muted-foreground mb-6">
        {text.earned} <span className="font-bold text-secondary">{coins}</span> {text.forCreativity}
      </p>

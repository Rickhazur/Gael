    ...
    return new Promise((resolve) => {
        const req = https.request(options, (res) => {
            ...
            fileStream.on('finish', () => {
                console.log(`✅ Generado con éxito: ${filename}`);
                resolve(true);
            });
        });
        req.write(data);
...
async function main() {
    console.log("🎙️ Grabando presentación del Tour con la voz de Lina...");
    for (const [file, text] of Object.entries(SCRIPTS)) {
        await generate(text, file);
    }
    console.log("✨ ¡Audios de presentación listos!");
}

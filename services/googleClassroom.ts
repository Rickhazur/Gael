...
// Hardcoding ID for debugging production
const CLIENT_ID = import.meta.env.VITE_GOOGLE_CLIENT_ID || '';
const CLIENT_SECRET = import.meta.env.VITE_GOOGLE_CLIENT_SECRET || '';
// Hardcoding production URI to avoid Vercel ENV mismatches
// Hardcoding production URI to avoid Vercel ENV mismatches
const REDIRECT_URI = 'https://www.novaschola.co/api/google/callback';
...
export function getGoogleAuthUrl(): string {
    const params = new URLSearchParams({
        ...
        scope: SCOPES,
        // access_type: 'offline', // Not available in implicit flow
        prompt: 'consent'
    });
...
export async function exchangeCodeForTokens(code: string) {
    ...
    if (!response.ok) {
        ...
        console.error('Google Exchange Error:', errorData);
        throw new Error(`Google Error: ${errorData.error} - ${errorData.error_description}`);
    }
...
export async function fetchCourses(accessToken: string) {
    // Fetch ALL courses, regardless of state (ACTIVE, ARCHIVED, PROVISIONED)
...
export async function fetchCourseWork(accessToken: string, courseId: string) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to fetch course work: ${err.error?.message || response.statusText} (Code: ${response.status})`);
    }
...
export async function fetchAllAssignments(accessToken: string) {
    console.log('DEBUG: Iniciando fetchAllAssignments...');
    const courses = await fetchCourses(accessToken);
    console.log(`DEBUG: Encontrados ${courses.length} cursos.`);

    const allAssignments = [];

    for (const course of courses) {
        console.log(`DEBUG: Procesando curso: ${course.name} (${course.id})`);
        try {
            const coursework = await fetchCourseWork(accessToken, course.id);
            console.log(`DEBUG: Curso ${course.name} tiene ${coursework.length} tareas.`);
            ...
            console.error(`DEBUG: Error en curso ${course.id}:`, e);
        }
    }

    console.log(`DEBUG: Total tareas encontradas: ${allAssignments.length}`);

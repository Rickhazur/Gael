...
export type MathStep = {
    ...
    visualType?: 'interactive_counting' | 'generic_illustration' | 'fraction_bar' | 'interactive_clock';
    visualData?: {
        items?: any[];
        targetCount?: number;
        numerator?: number;
...
export const MathSolver = {
    ...
    solveRectangleArea: (length: number, width: number): MathStep[] => {
        ...
        steps.push({
            ...
            boardDraw: { type: 'writeAnswer', value: length * width }
        });
    ...
    solveRectanglePerimeter: (length: number, width: number): MathStep[] => {
        ...
        steps.push({
            ...
            boardDraw: { type: 'writeAnswer', value: perimeter }
        });
    ...
    // GEOMETRY - Circle Circumference (2πr) - Socratic
    solveCircleCircumference: (radius: number): MathStep[] => {
        const steps: MathStep[] = [];
        const twoPiR = Math.round(2 * Math.PI * radius * 100) / 100;
        steps.push({
            id: 1,
            type: 'explanation',
            text: {
                es: `Vamos a calcular la circunferencia de un círculo con radio ${radius}. La fórmula es 2 × π × radio.`,
                en: `Let's calculate the circumference of a circle with radius ${radius}. The formula is 2 × π × radius.`
            }
        });
        const twoR = 2 * radius;
        steps.push({
            ...
            type: 'question',
            text: {
                es: `Primero calcula 2 × radio: 2 × ${radius} = ?`,
                en: `First calculate 2 × radius: 2 × ${radius} = ?`
            },
            expectedAnswer: twoR,
            failureHints: { es: [`2 × ${radius} = ${twoR}`], en: [`2 × ${radius} = ${twoR}`] }
        });
        steps.push({
            id: 3,
            type: 'question',
            text: {
                es: `Ahora multiplica por π (3.14): ${twoR} × 3.14 ≈ ?`,
                en: `Now multiply by π (3.14): ${twoR} × 3.14 ≈ ?`
            },
            expectedAnswer: twoPiR,
            failureHints: { es: [`La circunferencia es aproximadamente ${twoPiR}`], en: [`The circumference is approximately ${twoPiR}`] },
            boardDraw: { type: 'writeAnswer', value: twoPiR }
        });
        return steps;
    },

    // GEOMETRY - Cube Volume (lado³) - Socratic
    solveCubeVolume: (side: number): MathStep[] => {
        const steps: MathStep[] = [];
        ...
        steps.push({
            ...
            type: 'explanation',
            text: {
                es: `Vamos a calcular el volumen de un cubo de lado ${side}. El volumen es lado × lado × lado.`,
                en: `Let's calculate the volume of a cube with side ${side}. Volume is side × side × side.`
            }
        });
        steps.push({
            id: 2,
            type: 'question',
            text: {
                es: `¿Cuánto es ${side} × ${side} × ${side}?`,
                en: `What is ${side} × ${side} × ${side}?`
            },
            expectedAnswer: volume,
            failureHints: { es: [`${side}³ = ${volume}`], en: [`${side}³ = ${volume}`] },
            boardDraw: { type: 'writeAnswer', value: volume }
        });
        return steps;
    },

    // GEOMETRY - Sum of interior angles of a triangle (180°) - Conceptual
    solveTriangleAngleSum: (): MathStep[] => {
        const steps: MathStep[] = [];
        steps.push({
            id: 1,
            type: 'explanation',
            text: {
                es: `En cualquier triángulo, la suma de sus tres ángulos internos siempre es la misma.`,
                en: `In any triangle, the sum of its three interior angles is always the same.`
            }
        });
        steps.push({
            id: 2,
            type: 'question',
            text: {
                es: `¿Cuántos grados suman los tres ángulos de un triángulo? (Pista: es un número redondo.)`,
                en: `How many degrees do the three angles of a triangle add up to? (Hint: it's a round number.)`
            },
            expectedAnswer: 180,
            failureHints: { es: ['La suma es 180°. Puedes comprobarlo recortando un triángulo y juntando sus esquinas.'], en: ['The sum is 180°. You can check by cutting a triangle and putting the corners together.'] },
            boardDraw: { type: 'writeAnswer', value: 180 }
        });
        return steps;
    },

    // GEOMETRY - Pythagoras: hypotenuse from legs a, b (c² = a² + b²)
    solvePythagoras: (a: number, b: number): MathStep[] => {
        const steps: MathStep[] = [];
        const a2 = a * a;
        const b2 = b * b;
        const cSquared = a2 + b2;
        const c = Math.round(Math.sqrt(cSquared));
        steps.push({
            id: 1,
            type: 'explanation',
            text: {
                es: `En un triángulo rectángulo, el cuadrado de la hipotenusa es igual a la suma de los cuadrados de los catetos: c² = a² + b².`,
                en: `In a right triangle, the square of the hypotenuse equals the sum of the squares of the legs: c² = a² + b².`
            }
        });
        steps.push({
            id: 2,
            type: 'question',
            text: {
                es: `Primero calcula a²: ${a} × ${a} = ?`,
                en: `First calculate a²: ${a} × ${a} = ?`
            },
            expectedAnswer: a2,
            failureHints: { es: [`${a}² = ${a2}`], en: [`${a}² = ${a2}`] }
        });
        steps.push({
            id: 3,
            type: 'question',
        ...
        steps.push({
            ...
            expectedAnswer: cSquared,
            failureHints: { es: [`${a2} + ${b2} = ${cSquared}`], en: [`${a2} + ${b2} = ${cSquared}`] }
        });
        steps.push({
            id: 5,
            type: 'question',
            text: {
                es: `¿Qué número al cuadrado da ${cSquared}? Ese es la hipotenusa c.`,
                en: `What number squared gives ${cSquared}? That is the hypotenuse c.`
            },
            expectedAnswer: c,
            failureHints: { es: [`√${cSquared} = ${c}. La hipotenusa es ${c}.`], en: [`√${cSquared} = ${c}. The hypotenuse is ${c}.`] },
            boardDraw: { type: 'writeAnswer', value: c }
        });
        return steps;
    },
